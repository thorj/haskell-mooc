...
oddFactorial 0 = 1
oddFactorial n = if even n then 1 * oddFactorial (n-1) else n * oddFactorial (n-1)  
...
wspaces' :: Int -> String -> String
wspaces' n xs
    | n == 0 = xs
    | otherwise = wspaces' (n-1) xs ++ " "
...
countdown' 0 xs = xs ++ "Liftoff!" 
countdown' n xs = countdown' (n-1) ys
    where ys = xs ++ countdownprint' n  

countdownprint' :: Integer -> String
countdownprint' n = show n ++ "... "
------------------------------------------------------------------------------
