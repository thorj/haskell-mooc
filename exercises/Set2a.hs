...
takeFinal :: Int -> [a] -> [a]
takeFinal n xs = if n > lxs then xs else drop (lxs - n) xs 
    where lxs = length xs
------------------------------------------------------------------------------
...
substring i j s = take k (drop i s)
    where k = (j - i)
...
palindromify [] = ""
palindromify xs = if isPalindrome xs then xs else palindromify (tail (init xs)) 
...
safeIndex xs i = if i < 0 || i >= len then Nothing else Just (xs !! i) 
    where len = length xs
